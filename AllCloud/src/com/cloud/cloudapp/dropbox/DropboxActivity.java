...
public class DropboxActivity extends Activity implements OnItemClickListener {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			if (msg.what == 0) {
				lvDropboxDownloadFilesList.setAdapter(new DropboxFileListAdapter(
						DropboxActivity.this, files));
				pd.dismiss();
			} else if (msg.what == 1) {
	...
	private void checkAppKeySetup() {
		...
		String scheme = "db-" + DropBoxConstants.DROPBOX_APP_KEY;
	...
	private AndroidAuthSession buildSession() {
		AppKeyPair appKeyPair = new AppKeyPair(DropBoxConstants.DROPBOX_APP_KEY,
				DropBoxConstants.DROPBOX_APP_SECRET);
		...
		if (stored != null) {
			AccessTokenPair accessToken = new AccessTokenPair(stored[0],
					stored[1]);
			session = new AndroidAuthSession(appKeyPair, DropBoxConstants.ACCESS_TYPE,
					accessToken);
		} else {
			session = new AndroidAuthSession(appKeyPair, DropBoxConstants.ACCESS_TYPE);
		}
	...
	public void setLoggedIn(final boolean loggedIn) {
		pd = ProgressDialog.show(DropboxActivity.this, null,
				"Retrieving data...");
		new Thread(new Runnable() {

			@Override
			public void run() {
				DropBoxConstants.mLoggedIn = loggedIn;
				if (loggedIn) {
					int i = 0;
	...
	private void storeKeys(String key, String secret) {
		SharedPreferences prefs = getSharedPreferences(
				DropBoxConstants.ACCOUNT_PREFS_NAME, 0);
		Editor edit = prefs.edit();
		edit.putString(DropBoxConstants.ACCESS_KEY_NAME, key);
		edit.putString(DropBoxConstants.ACCESS_SECRET_NAME, secret);
		edit.commit();
	}
	...
	private String[] getKeys() {
		SharedPreferences prefs = getSharedPreferences(
				DropBoxConstants.ACCOUNT_PREFS_NAME, 0);
		String key = prefs.getString(DropBoxConstants.ACCESS_KEY_NAME, null);
		String secret = prefs.getString(DropBoxConstants.ACCESS_SECRET_NAME, null);
	...
	private void copy(final Entry fileSelected, final File localFile) {
		final ProgressDialog pd = ProgressDialog.show(DropboxActivity.this,
	...
	private void showFileExitsDialog(final Entry fileSelected,
		...
		AlertDialog.Builder alertBuilder = new Builder(DropboxActivity.this);
		alertBuilder.setMessage(DropBoxConstants.OVERRIDEMSG);
