...
public class BoxActivity extends Activity {
    ...
    private static final String SHARED_PREF_NAME = BoxActivity.class.getName();
        ...
        if (Activity.RESULT_OK != resultCode) {
            ...
            AsyncTask<Null, Integer, Null> task = new AsyncTask<Null, Integer, Null>() {
                ...
                protected void onPostExecute(Null result) {
                    Toast.makeText(BoxActivity.this, "done downloading", Toast.LENGTH_LONG).show();
                    super.onPostExecute(result);
                }

                @Override
                protected void onPreExecute() {
                    Toast.makeText(BoxActivity.this, "start downloading", Toast.LENGTH_LONG).show();
                    super.onPreExecute();
                }

                @Override
                protected Null doInBackground(Null... params) {
                    BoxAndroidClient client = ((AllCloudApplication) getApplication()).getClient();
                    try {
    ...
    private void onFolderSelected(int resultCode, Intent data) {
        if (Activity.RESULT_OK != resultCode) {
            ...
            AsyncTask<Null, Integer, Null> task = new AsyncTask<Null, Integer, Null>() {
                ...
                @Override
                protected void onPostExecute(Null result) {
                    Toast.makeText(BoxActivity.this, "done uploading", Toast.LENGTH_LONG).show();
                    super.onPostExecute(result);
                }

                @Override
                protected void onPreExecute() {
                    Toast.makeText(BoxActivity.this, "start uploading", Toast.LENGTH_LONG).show();
                    super.onPreExecute();
                }
    ...
    private void doUpload() {
        try {
            BoxAndroidClient client = ((AllCloudApplication) getApplication()).getClient();
            Intent intent = FolderPickerActivity.getLaunchIntent(this, "0", (BoxAndroidOAuthData) client.getAuthData(), AllCloudApplication.CLIENT_ID,
            		AllCloudApplication.CLIENT_SECRET);
    ...
    private void doDownload() {
        try {
            BoxAndroidClient client = ((AllCloudApplication) getApplication()).getClient();
            Intent intent = FilePickerActivity.getLaunchIntent(this, "0", (BoxAndroidOAuthData) client.getAuthData(), AllCloudApplication.CLIENT_ID,
            		AllCloudApplication.CLIENT_SECRET);
    ...
    private void startAuthenticationFromUI() {
        Intent intent = OAuthActivity.createOAuthActivityIntent(this, AllCloudApplication.CLIENT_ID, AllCloudApplication.CLIENT_SECRET, false,
        		AllCloudApplication.REDIRECT_URL);
    ...
    private void authenticate(BoxAndroidOAuthData auth) {
        BoxAndroidClient client = new BoxAndroidClient(AllCloudApplication.CLIENT_ID, AllCloudApplication.CLIENT_SECRET, null, null, null);
        client.authenticate(auth);
        ((AllCloudApplication) getApplication()).setClient(client);
        saveAuth(auth);
    ...
    private void saveAuth(BoxAndroidOAuthData auth) {
        try {
            IBoxJSONParser parser = ((AllCloudApplication) getApplication()).getJSONParser();
